<template>
    <div class="index-content">
        <div class="top">
            <div>
                <van-icon is-link @click="showleft" size='25' :badge="topBadge" name="bars" />
                <van-popup v-model="show" position="left" >
                    <div :style="{width:Pwidth*0.8+'px',height:Pheight+'px',color:'red'}" class="left-content">
                        <CBL></CBL>
                    </div>    
                </van-popup>
            </div>
            <div>
                <van-search v-model="searchValue" @input="autoSearch"  background="transparent"  shape="round" placeholder="请输入搜索关键词" />
            </div>
            <div>
            ...
            <div class="center">
              <ul v-show="isSearchList" class="searchList" :style="{width:Pwidth+'px',height:Pheight*0.4+'px'}">
                    <li v-for="(item) in allArea"
                    :key="item.id">{{item.cp}}</li>
                </ul>
            <!-- 轮播图组件 -->
            <van-swipe :style="{width:Pwidth*0.9+'px',margin:'auto',borderRadius:'19px'}" height='180' :autoplay="3000">
            <van-swipe-item v-for="(image, index) in images" :key="index">
            <img :style="{width:Pwidth+'px',height:'100%'}" v-lazy="image" />
            </van-swipe-item>
            </van-swipe>
            <!-- 各个板块 -->
            <div class="ggbk" :style="{overflow:'scroll'}">
                <ul class="bklist" :style="{width:Pwidth*bkpages+'px'}">
                    <li :style="{width:Pwidth/5+'px'}"   v-for="(item) in gnlist" :key='item.id'>
                        <i @click="$router.push(`/${item.path}`)" class="iconfont" :class="[item.tp]"></i>
...
export default {
    ...
    data(){
        return {
            ...
            searchValue:'',
            allArea:[],
            allNewArea:[{id:'1',uname:'lyc',cp:'dxw'},{id:'2',uname:'lyc',cp:'dxw2'},{id:'3',uname:'lyc',cp:'dxw3'}],
            isSearchList:false,
            //轮播图
            images: [
      ...
      //模糊搜索功能
        autoSearch () { // 模糊搜索加节流（500ms触发一次）      
            ...
            setTimeout(() => {
                ...
                if (item.cp.indexOf(this.searchValue) !== -1) { // 此处也可使用js的 search 方法实现indexOf 一样效果
